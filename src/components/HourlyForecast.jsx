  ...
  const formattedHours = useMemo(() => {
    ...
    return hourly.slice(0, 24).map((item, index) => {
      ...
      return {
        ...
        dateLabel: monthDay,
        temp: Math.round(unit === 'imperial' ? item.temp * 9 / 5 + 32 : item.temp),
        tempC: Math.round(item.temp),
        feelsLike: Math.round(unit === 'imperial' ? item.feels_like * 9 / 5 + 32 : item.feels_like),
        icon: weather?.id ?? 800,
